...
describe('Tool Registration', () => {
  ...
  it('should register all GitLab tools', async () => {
    ...
    const [name, config, handler] = mockServer.registerTool.mock.calls.find((call: any) => call[0] === 'gitlab_list_mrs');
  ...
  it('should register all GitHub tools', async () => {
    ...
    // Test one tool handler to ensure it calls the adapter
    const [name, config, handler] = mockServer.registerTool.mock.calls.find((call: any) => call[0] === 'github_list_prs');
  ...
  it('should handle gitlab_get_mr_details correctly', async () => {
    registerGitLabTools(mockServer, mockGitLabAdapter);
    const [,, handler] = mockServer.registerTool.mock.calls.find((call: any) => call[0] === 'gitlab_get_mr_details');
  ...
  it('should handle gitlab_get_mr_diff correctly', async () => {
    registerGitLabTools(mockServer, mockGitLabAdapter);
    const [,, handler] = mockServer.registerTool.mock.calls.find((call: any) => call[0] === 'gitlab_get_mr_diff');
  ...
  it('should handle gitlab_read_file correctly', async () => {
    registerGitLabTools(mockServer, mockGitLabAdapter);
    const [,, handler] = mockServer.registerTool.mock.calls.find((call: any) => call[0] === 'gitlab_read_file');
  ...
  it('should handle gitlab_get_project_metadata correctly', async () => {
    registerGitLabTools(mockServer, mockGitLabAdapter);
    const [,, handler] = mockServer.registerTool.mock.calls.find((call: any) => call[0] === 'gitlab_get_project_metadata');
  ...
  it('should handle github_get_pr_details correctly', async () => {
    registerGitHubTools(mockServer, mockGitHubAdapter);
    const [,, handler] = mockServer.registerTool.mock.calls.find((call: any) => call[0] === 'github_get_pr_details');
  ...
  it('should handle github_get_pr_diff correctly', async () => {
    registerGitHubTools(mockServer, mockGitHubAdapter);
    const [,, handler] = mockServer.registerTool.mock.calls.find((call: any) => call[0] === 'github_get_pr_diff');
  ...
  it('should handle github_read_file correctly', async () => {
    registerGitHubTools(mockServer, mockGitHubAdapter);
    const [,, handler] = mockServer.registerTool.mock.calls.find((call: any) => call[0] === 'github_read_file');
    
    await handler({ repoId: 'owner/repo', filePath: 'README.md', ref: 'main' });
    expect(mockGitHubAdapter.readFileContent).toHaveBeenCalledWith('owner/repo', 'README.md', 'main');
  });

  it('should handle github_get_project_metadata correctly', async () => {
    registerGitHubTools(mockServer, mockGitHubAdapter);
    const [,, handler] = mockServer.registerTool.mock.calls.find((call: any) => call[0] === 'github_get_project_metadata');
